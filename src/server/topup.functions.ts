...
/**
 * Create a real Stripe Checkout Session for a coin package.
 * Returns the hosted Stripe Checkout URL — the client redirects to it.
 * Coins are NOT credited here. They are credited only after we verify the
 * session was actually paid (see `verifyCoinCheckout`).
 */
export const createCoinCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Record the pending session so we can credit coins exactly once on success.
 ...
 * Credits coins only when:
 *  - the session belongs to the calling user (metadata.userId match)
 *  - Stripe says payment_status === "paid"
 *  - the session has not already been credited
 *
...
export const verifyCoinCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Look up our record (created on session create).
