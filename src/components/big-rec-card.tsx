...
/**
 * Tall vertical "More like this" recommendation banner.
 * Fills space with a hero-like image, gradient veil, and rich metadata.
 */
  ...
  return (
    <Link
      ...
      className="group glow-halo relative block overflow-hidden rounded-2xl shadow-card ring-1 ring-border/50
                 transition-all duration-500 ease-out hover:-translate-y-1 hover:shadow-elev"
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-4 space-y-2">
        <span className="inline-block rounded-md bg-black/55 px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wider text-white ring-1 ring-white/10 backdrop-blur-md">
        ...
        <div className="flex items-center gap-3 pt-1 text-[10px] text-white/65 font-bold uppercase tracking-wider">
          <span className="inline-flex items-center gap-1"><Eye className="h-3 w-3" /> {series.views?.toLocaleString() ?? 0}</span>
          <span className="inline-flex items-center gap-1"><BookOpen className="h-3 w-3" /> Read</span>
        </div>
        {series.genres?.[0] && (
