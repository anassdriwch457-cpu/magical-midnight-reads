...
async function uploadAdminFile(file: File, path: string) {
  ...
  const res = await api.post<{ url: string; path?: string }>("/admin/uploads", form);
  return res.url;
}

async function loadSeriesList() {
  return api.get<{ data: Series[]; total?: number; meta?: unknown }>(
    "/series?sort=created_at&dir=desc&per_page=1000",
  );
}

async function loadSeriesById(id: string) {
  return api.get<Series>(`/series/${id}`);
}

async function loadChapters(seriesId: string) {
  return api.get<Chapter[]>(`/series/${seriesId}/chapters`);
}
    ...
    <div className="pt-20 min-h-screen bg-gradient-to-br from-background via-background to-primary/5">
      <div className="flex">
        <AdminSidebar
          ...
          roleLabel={
            isSuperAdmin ? "Super-Admin" : isManager ? "Manager" : isUploader ? "Uploader" : "Staff"
          }
        />
  ...
  return (
    <aside className="hidden md:flex w-60 shrink-0 flex-col border-r border-border/40 bg-card/30 backdrop-blur-xl min-h-[calc(100vh-5rem)] p-4 gap-1 supports-[backdrop-filter]:bg-card/20">
      <div className="px-3 py-4 mb-2 border-b border-border/40">
        ...
        <div className="mt-1 text-lg font-extrabold tracking-tight bg-gradient-to-r from-foreground to-primary bg-clip-text text-transparent">
          Control Center
        </div>
      </div>
      {items
        .filter((i) => allowedTabs.includes(i.key))
...
function AnalyticsView() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const [overview, top, growth, revenue] = await Promise.all([
          api.get<{
            total_sales: number;
          ...
          >(`/admin/analytics/top-series?limit=10`),
          api.get<GrowthDay[]>(`/admin/analytics/user-growth?days=30`),
          api.get<RevenueDay[]>(`/admin/analytics/revenue-daily?days=30`),
        ]);
        setTotalSales(Number(overview.total_sales ?? 0));
        setTopSeries((top ?? []) as never);
        const growthDays = (growth ?? []).map((d) => ({ ...d, signups: Number(d.signups) }));
        setGrowthSeries(growthDays);
        setGrowth30(growthDays.reduce((a, d) => a + d.signups, 0));
        setGrowth7(growthDays.slice(-7).reduce((a, d) => a + d.signups, 0));
        const revenueDays = (revenue ?? []).map((d) => ({
          ...d,
          revenue: Number(d.revenue),
          unlocks: Number(d.unlocks),
        }));
        setRevenueSeries(revenueDays);
        const last7 = revenueDays.slice(-7);
  ...
  return (
    <div>
      <SectionHeader
        icon={BarChart3}
        title="Dashboard"
        subtitle="Real-time revenue, growth, and engagement"
      />
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        <GlassCard
          ...
          subtitle={`${revenue30Total(revenueSeries).toLocaleString()} coins total`}
        >
        ...
        <GlassCard title="Growth & Unlocks (30d)" subtitle="Signups vs chapter unlocks">
          ...
          <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-sm bg-primary inline-block" /> Signups
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-sm bg-muted-foreground inline-block" /> Unlocks
            </span>
          </div>
        </GlassCard>
      </div>
              ...
              <li key={s.series_id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {Number(s.unlocks)} unlocks · {Number(s.revenue).toLocaleString()} coins
                  </div>
                </div>
              </li>
            ))}
...
function StatCard({
  ...
  return (
    <div
      className={`rounded-xl border backdrop-blur-xl p-5 shadow-lg transition-all hover:scale-[1.02] ${accent ? "border-primary/40 bg-gradient-to-br from-primary/10 to-primary/5 shadow-primary/10" : "border-border/50 bg-card/40 shadow-black/5"}`}
    >
      ...
      <div className={`mt-2 text-2xl font-extrabold tabular-nums ${accent ? "text-primary" : ""}`}>
        {value}
      </div>
    </div>
  );
...
function ContentView() {
  const [view, setView] = useState<
    { kind: "list" } | { kind: "edit"; id: string | null } | { kind: "chapters"; id: string }
  >({ kind: "list" });
  return (
    <div>
      {view.kind === "list" && (
        <SeriesList
          ...
          onChapters={(id) => setView({ kind: "chapters", id })}
          onNew={() => setView({ kind: "edit", id: null })}
        />
      )}
      {view.kind === "edit" && <SeriesForm id={view.id} onBack={() => setView({ kind: "list" })} />}
      {view.kind === "chapters" && (
        <ChapterManager seriesId={view.id} onBack={() => setView({ kind: "list" })} />
      )}
    </div>
  );
}
...
function SeriesList({
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await loadSeriesList();
      setItems(res.data ?? []);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Failed to load series");
    }
    setSelected(new Set());
    setLoading(false);
  };
  ...
  const bulkFlag = async (field: "is_trending" | "is_popular", value: boolean) => {
    ...
    try {
      const res = await api.post<{ success: boolean; updated?: number; error?: string }>(
        "/admin/series/bulk-flags",
        {
          ids,
          is_trending: field === "is_trending" ? value : undefined,
          is_popular: field === "is_popular" ? value : undefined,
        },
      );
  ...
  const bulkDelete = async () => {
    if (selected.size === 0) return toast.error("Nothing selected");
    if (!confirm(`Delete ${selected.size} series and all their chapters? This cannot be undone.`))
      return;
    setBulkBusy(true);
    try {
      await api.post("/admin/series/bulk-delete", { ids: Array.from(selected) });
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <div className="flex gap-1 rounded-md border border-border/50 bg-card/40 backdrop-blur-xl p-1 ml-auto">
          <button
            ...
            className={`p-1.5 rounded ${view === "grid" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
            title="Grid view"
          >
          ...
          <button
            ...
            className={`p-1.5 rounded ${view === "list" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
            title="List view"
          >
      ...
      {selected.size > 0 && (
        <div className="rounded-xl border border-primary/40 bg-gradient-to-r from-primary/15 to-primary/5 backdrop-blur-xl p-3 mb-4 flex items-center gap-2 flex-wrap">
          ...
          <Button
            ...
            variant="outline"
            disabled={bulkBusy}
            onClick={() => bulkFlag("is_trending", true)}
          >
            <Flame className="h-3.5 w-3.5" /> Mark Trending
          </Button>
          <Button
            size="sm"
            variant="outline"
            disabled={bulkBusy}
            onClick={() => bulkFlag("is_trending", false)}
          >
            Untrending
          </Button>
          <Button
            ...
            variant="outline"
            disabled={bulkBusy}
            onClick={() => bulkFlag("is_popular", true)}
          >
            <Sparkles className="h-3.5 w-3.5" /> Mark Popular
          </Button>
          <Button
            size="sm"
            variant="outline"
            disabled={bulkBusy}
            onClick={() => bulkFlag("is_popular", false)}
          >
            Unpopular
          </Button>
          <Button
      ...
      {loading ? (
        ...
        <p className="text-muted-foreground py-8 text-center">
          {search ? "No matches." : "No series yet. Create your first one."}
        </p>
      ) : view === "grid" ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {filtered.map((s) => {
            ...
            return (
              <div
                key={s.id}
                className={`group relative rounded-xl overflow-hidden border backdrop-blur-xl transition-all ${sel ? "border-primary ring-2 ring-primary/40" : "border-border/50 bg-card/40 hover:border-primary/40"}`}
              >
                <button
                  onClick={() => toggleSelect(s.id)}
                  className="absolute top-2 left-2 z-10 h-6 w-6 rounded-md bg-background/80 backdrop-blur border border-border/60 flex items-center justify-center"
                  aria-label="Select"
                >
                  <div
                    className={`h-3.5 w-3.5 rounded-sm ${sel ? "bg-primary" : "bg-transparent border border-border"}`}
                  />
                </button>
                <div className="absolute top-2 right-2 z-10 flex gap-1">
                  {s.is_trending && (
                    <span
                      title="Trending"
                      className="h-6 w-6 rounded-md bg-orange-500/90 text-white flex items-center justify-center"
                    >
                  ...
                  {s.is_popular && (
                    <span
                      title="Popular"
                      className="h-6 w-6 rounded-md bg-primary/90 text-primary-foreground flex items-center justify-center"
                    >
                ...
                <div className="p-2.5">
                  ...
                  <div className="flex gap-1 mt-2">
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      className="h-7 px-1.5 ml-auto text-destructive hover:text-destructive"
                ...
                <tr key={s.id} className="border-t border-border/40 hover:bg-card/60">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="inline-flex gap-1">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => onEdit(s.id)} title="Edit">
...
function SeriesForm({ id, onBack }: { id: string | null; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await loadSeriesById(id);
        setForm({
          title: data.title,
          slug: data.slug,
  ...
  const save = async () => {
    ...
    try {
      if (coverFile) {
        ...
        coverUrl = await uploadAdminFile(coverFile, path);
      }
      const payload = {
      ...
      if (id) {
        await api.patch(`/admin/series/${id}`, payload);
  ...
  return (
    <div>
      ...
      <h2 className="text-lg font-bold uppercase tracking-wider mb-5">
        {id ? "Edit Series" : "New Series"}
      </h2>
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select
                value={form.type}
                onValueChange={(v) => setForm((f) => ({ ...f, type: v as "manga" | "novel" }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                value={form.status}
                onValueChange={(v) =>
                  setForm((f) => ({ ...f, status: v as "ongoing" | "completed" | "hiatus" }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
...
function ChapterManager({ seriesId, onBack }: { seriesId: string; onBack: () => void }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [s, c] = await Promise.all([loadSeriesById(seriesId), loadChapters(seriesId)]);
      setSeries(s ?? null);
      setChapters(c ?? []);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Failed to load chapters");
    } finally {
      setLoading(false);
    }
  };
  ...
  const updatePrice = async (id: string, newPrice: number) => {
    try {
      await api.patch(`/admin/chapters/${id}`, { price: newPrice });
  ...
  const createChapter = async () => {
    ...
    if (series?.type === "manga" && !zipFile)
      return toast.error("Please select a ZIP file with chapter images");
    if (series?.type === "novel" && !content.trim())
      return toast.error("Please add the chapter text");

    setCreating(true);
    try {
      const ch = await api.post<Chapter>(`/admin/series/${seriesId}/chapters`, {
        number: n,
        chapter_number: n,
      ...
      if (series?.type === "manga" && zipFile) {
        ...
        for (let i = 0; i < imageEntries.length; i++) {
          ...
          const file = new File([blob], entry.name, { type: contentType });
          const imageUrl = await uploadAdminFile(file, path);
          pages.push({ page_number: i + 1, image_url: imageUrl });
        }
        ...
        await api.put(`/admin/chapters/${ch.id}/pages`, { pages });
      }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        {series.cover_url && (
          <img src={series.cover_url} alt="" className="h-14 w-10 object-cover rounded-sm" />
        )}
        <div>
          ...
          {series.type === "manga" ? (
            <div className="md:col-span-2">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Images sorted naturally. JPG, PNG, WEBP, GIF, AVIF supported.
              </p>
            </div>
          ) : (
      ...
      {chapters.length === 0 ? (
        <p className="text-muted-foreground text-sm py-6 text-center">No chapters yet.</p>
      ) : (
                ...
                <tr key={c.id} className="border-t border-border">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => removeChapter(c.id, Number(c.number))}
                      className="text-destructive hover:text-destructive"
                    >
                      <Trash2 className="h-4 w-4" />
                    </Button>
  ...
  return (
    <div>
      <SectionHeader
        icon={UsersIcon}
        title="Users"
        subtitle={`${stats.total} total · ${stats.staff} staff · ${stats.banned} banned`}
      />
              ...
              <tbody>
                {filtered.map((u) => {
                  ...
                  return (
                    <tr
                      key={u.id}
                      className={`border-t border-border/40 hover:bg-card/60 transition-colors ${banned ? "opacity-60" : ""}`}
                    >
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 font-bold tabular-nums">
                        <span className="inline-flex items-center gap-1">
                          <Coins className="h-3.5 w-3.5 text-primary" /> {u.coins}
                        </span>
                      </td>
...
function UserModal({
  ...
  const adjust = async () => {
    ...
    try {
      const res = await api.post<{ success: boolean; balance?: number; error?: string }>(
        `/admin/users/${user.id}/coins`,
        { delta: d, reason: reason.trim() || undefined },
      );
      if (!res.success) return toast.error(res.error ?? "Failed");
      ...
      toast.error(e instanceof Error ? e.message : "Coin adjustment failed");
    } finally {
      setBusy(false);
    }
  };
  ...
  const toggleRole = async (role: "admin" | "manager" | "super_admin", grant: boolean) => {
    setBusy(true);
    try {
      const res = await api.post<{ success: boolean; error?: string; roles?: string[] }>(
      ...
      setRoles(
        res.roles ??
          (grant ? Array.from(new Set([...roles, role])) : roles.filter((r) => r !== role)),
      );
  ...
  const toggleBan = async () => {
    const next = !banned;
    if (next && !confirm(`Ban ${user.email}? They will be signed out and unable to log back in.`))
      return;
    setBusy(true);
    try {
      const res = await api.patch<{ success: boolean; banned_until: string | null }>(
      ...
      if (!res.success) return toast.error("Failed");
      setBanned(next);
      toast.success(next ? "User banned" : "User unbanned");
      onChanged();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Ban update failed");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    ...
    >
      <div
        className="bg-card/95 backdrop-blur-xl border border-border/50 rounded-xl max-w-lg w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto"
...
function FinanceView() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const data = await api.get<FinanceRow[]>("/admin/finance/log?limit=200");
        setRows(data ?? []);
      } catch (e) {
        toast.error(e instanceof Error ? e.message : "Failed to load finance log");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const filtered = rows.filter((r) => {
    ...
    const q = search.trim().toLowerCase();
    if (q && !(r.user_email ?? "").toLowerCase().includes(q) && !r.note.toLowerCase().includes(q))
      return false;
    return true;
  });
  ...
  const totals = useMemo(() => {
    ...
    const granted = rows
      .filter((r) => r.kind === "adjustment" && r.amount > 0)
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <StatCard
          icon={ArrowUpRight}
          label="Granted by Staff"
          value={totals.granted.toLocaleString()}
        />
        <StatCard
          icon={ArrowDownRight}
          label="Removed by Staff"
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((r, i) => (
                  <tr
                    key={i}
                    className="border-t border-border/40 hover:bg-card/60 transition-colors"
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="md:hidden text-[10px] text-muted-foreground truncate max-w-[180px]">
                        {r.note}
                      </div>
                    </td>
                    ...
                    <td
                      className={`px-4 py-3 text-right font-bold tabular-nums ${r.amount < 0 ? "text-destructive" : "text-primary"}`}
                    >
                      {r.amount > 0 ? "+" : ""}
...
function MassPriceTool({
  ...
  const apply = async () => {
    ...
    if (
      !confirm(
        `Set the price of ALL ${chapterCount} chapter(s) to ${p} coin(s)? This cannot be undone.`,
      )
    )
      return;
    setBusy(true);
    try {
      const res = await api.post<{ success: boolean; updated?: number; error?: string }>(
        `/admin/series/${seriesId}/chapters/mass-update-price`,
        { price: p },
      );
      if (!res.success) return toast.error(res.error ?? "Failed");
      ...
      toast.error(e instanceof Error ? e.message : "Mass update failed");
    } finally {
      setBusy(false);
    }
  };
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Set the same coin price across every chapter in this series.
          </p>
        </div>
      </div>
    ...
    (async () => {
      try {
        const [s, list] = await Promise.all([
          ...
          api.get<{ data: { id: string; title: string }[] }>(
            "/series?sort=title&dir=asc&per_page=1000",
          ),
        ]);
        if (s)
          setSettings({
            ...
            hero_series_id: s.hero_series_id,
          });
  ...
  return (
    <div className="max-w-2xl">
      <SectionHeader
        icon={SettingsIcon}
        title="Site Settings"
        subtitle="Brand, SEO, and the hero banner"
      />
      ...
      <div className="space-y-5">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="Nuvia Toon"
              />
              <p className="text-xs text-muted-foreground mt-1">
                Shown across the homepage and SEO meta.
              </p>
            </div>
            <div>
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                Used as the meta description on the homepage.
              </p>
            </div>
          </div>
        </GlassCard>
        ...
        <GlassCard
          title="Hero Banner Series"
          subtitle="Pick the headline series for the homepage hero"
        >
          <Select
            value={settings.hero_series_id ?? "__none__"}
            onValueChange={(v) =>
              setSettings((s) => ({ ...s, hero_series_id: v === "__none__" ? null : v }))
            }
          >
            <SelectTrigger>
              <SelectValue placeholder="No hero" />
            </SelectTrigger>
            <SelectContent>
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Saving will mark this series as the only Trending one.
          </p>
        </GlassCard>
